...
**Audio spec (both sets, identical):** mono, 44.1 kHz, 16-bit WAV, peak-normalised to −1 dBFS.

---

## How to reproduce / run it

The experiment is a single self-contained `index.html` plus the two audio folders.
...
2. Put these in the **repository root**, with **exact, case-sensitive names** (the server is Linux):
   ...
   ```
3. In the Pavlovia dashboard, set the experiment status to Piloting.
...
5. Two NAD tasks, verbal and non-verbal, order counterbalanced, separated by a short break.
6. Post-task awareness assessment.
7. Debrief and return to Prolific.
...
- **Task order (verbal-first vs. non-verbal-first):** assigned by Prolific-ID parity, the sum of the ID's character codes; even → verbal first, odd → non-verbal first. With no Prolific ID (local testing), it falls back to random. The method used is stored in the data as `task_order_method` (`prolific_id_parity` or `random_no_id`).
